...
function sortByKey(ele1, ele2, order, key) {
...
app.get('/hotels/sort/pricing', (req, res) => {
  ...
  let arrCp = hotels.slice();
  let key = 'price';
  let result = arrCp.sort((a, b) => sortByKey(a, b, order, key));
...
function filterByKey(objele, key, val) {
  return objele[key].toLowerCase() === val.toLowerCase();
}
app.get('/hotels/filter/amenity', (req, res) => {
  let Pval = req.query.amenity;
  let arrCp = hotels.slice();
  let key = 'amenity';
  let result = arrCp.filter((eleObj) => filterByKey(eleObj, key, Pval));
  res.json({ hotels: result });
});

//Endpoint 5: Filter the hotels based on the selected Country
app.get('/hotels/filter/country', (req, res) => {
  let Pval = req.query.country;
  let arrCp = hotels.slice();
  let key = 'country';
  let result = arrCp.filter((eleObj) => filterByKey(eleObj, key, Pval));
  res.json({ hotels: result });
